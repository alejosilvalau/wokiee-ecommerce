...
export const getProduct = createAsyncThunk(
	...
	async (payload, thunkAPI) => {
		let url = isProd() + `/api/products`;
		if (payload.cat) {
			url = isProd() + `/api/products/?cat=${payload.cat}`;
		} else if (payload.price.priceL) {
			url =
				isProd() +
				`/api/products/?pricel=${payload.price.priceL}&&priceh=${payload.price.priceH}`;
		}
...
export const asyncFav = createAsyncThunk(
	"product/asyncFav",
	async ({ id, token }) => {
		const res = await fetch(isProd() + `/api/products/fav/${id}`, {
			method: "POST",
			headers: {
				token: token,
			},
...
export const asyncFavGet = createAsyncThunk(
	"product/asyncFavGet",
	async (token) => {
		const res = await fetch(isProd() + `/api/wishlist`, {
			headers: {
				token: token,
			},
		});
...
export const asyncSingleProduct = createAsyncThunk(
	"product/asyncSingleProduct",
	async (id, thunkApi) => {
		const res = await fetch(isProd() + `/api/products/${id}`);
		const data = await res.json();
		return data;
	}
);
